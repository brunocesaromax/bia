...
Antes de abrir qualquer PR, confirme que o `gh` está instalado e autenticado (`gh --version` e `gh auth status`). Se não estiver, avise o usuário e peça para instalar (https://cli.github.com/) e rodar `gh auth login` antes de prosseguir com a etapa de abertura de PR.
...
- Você deverá delegar a atividade para inicio de um desses subagentes Claude Code (invocados pelo nome via Task tool):
    - dev (`.claude/agents/dev.md`)
    - devops (`.claude/agents/devops.md`)
    - qa (`.claude/agents/qa.md`)
    - po (`.claude/agents/po.md`, você mesmo)

O local que o arquivo deve ser criado, será na pasta .claude/tasks
...
- Na task precisa estar claro que SEMPRE quem irá finalizar a task e mover ela para done seja você (po)
 ...
 - Precisa estar documentado essa etapa do que você deverá fazer ao final:
    ...
    - **Entrar no worktree para revisar:** `cd .claude/worktrees/XXX-tipo-resumo`
...
- `.claude/docs/task-template-with-worktree.md` - Template completo com worktree
- `.claude/docs/worktree-workflow.md` - Guia detalhado para alunos
- `.claude/docs/worktree-steering.md` - Steering file para agents
