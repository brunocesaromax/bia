...
### Interface e UX
- [x] Gráfico legível em tema claro e escuro. _(validado pelo QA no navegador)_
...
- [x] Após o Tailwind entrar (preflight/reset global), a home (`/`) e a rota `/about` continuam visualmente corretas — conferido no navegador, tema claro e escuro. _(validado pelo QA)_
...
- [x] Navegar home → gráfico → voltar; alterar prioridade e revisitar o gráfico. _(validado pelo QA)_
- [x] Testar em tema claro e escuro. _(validado pelo QA)_
- [x] Regressão pós-Tailwind: home (`/`) e `/about` inspecionadas no navegador — layout, formulário, lista de tarefas e paginação intactos. _(validado pelo QA)_
- [N/A] `npm test` do `client/` continua passando (jest/RTL já existente). _(premissa incorreta — não existe runner de testes no `client/`; ver Notas de Implementação e Decisões do PO)_
- [N/A] (Se aplicável) Teste unitário da função de agrupamento usando o setup jest/RTL já existente em `client/`. _(sem runner no client; `groupTasksByPriority` validada por script Node ad-hoc + comportamento validado ponta a ponta pelo QA)_

## 📚 Definição de Pronto (DoD)
- [x] Código implementado e testado
- [x] Todos os itens do checklist marcados ✅ _(itens `[N/A]` justificados)_
- [x] Commits descritivos e frequentes
- [x] Push do branch realizado
- [x] Nenhum arquivo fora de `client/` alterado
- [x] Home e `/about` validadas visualmente após entrada do Tailwind _(validado pelo QA)_
...
- [~] `docker compose up` sobe app + banco no worktree _(não executado no worktree — ambiente Docker em uso pela task 006 paralela, nomes de container fixos; build via Dockerfile validado com `docker compose build`; app validado no navegador pelo QA no ambiente principal)_

### Setup shadcn/ui + Tailwind no client
- [x] `npx shadcn@latest init` em `client/` (defaults Vite + React/JSX); commitar `tailwind.config.*`, config PostCSS, `components.json`, `client/src/lib/utils.js` _(Tailwind v4 não usa `tailwind.config.*`/PostCSS — usa `@tailwindcss/vite` + diretivas no CSS; `components.json` + `src/lib/utils.js` versionados)_
- [x] Integrar diretivas Tailwind + CSS variables no `client/src/index.css` / `main.jsx` sem apagar o CSS puro existente _(camadas theme+utilities, sem preflight; `@theme inline` mapeia para os tokens existentes)_
- [x] `npx shadcn@latest add chart` (gera `client/src/components/ui/chart.jsx`, adiciona `recharts` ao `package.json`) _(CLI quebrado no caminho Vite+JSX; componente `chart` oficial do registry aplicado manualmente, TSX→JSX)_
- [x] Subir o app e checar no navegador que home (`/`) e `/about` não regrediram com o preflight do Tailwind; ajustar config se necessário _(validado pelo QA)_
...
### Testes
- [N/A] `npm test` do `client/` passa _(não há runner de testes no `client/`)_
- [N/A] Teste unitário da função de agrupamento (se aplicável) _(validada por script Node ad-hoc + QA ponta a ponta)_
- [x] Testes manuais dos cenários da seção 🧪 realizados _(pelo QA, no navegador via Playwright)_

### Finalização
- [x] Código revisado
- [x] Commits finalizados com mensagens descritivas
- [x] Push do branch realizado
- [x] Todos os itens acima marcados ✅ _(itens `[N/A]`/`[~]` justificados)_
...
**Status: APROVADA e encerrada.** Implementada pelo `dev`, validada no navegador pelo `qa`
(Playwright, sem bugs). Worktree revisado pelo PO: diff limitado a `client/` + o `.md` desta task,
código limpo e bem escopado (lazy-load da rota `/prioridades` + `recharts`, sem preflight do
Tailwind, tokens de tema reaproveitados).

### Decisões do PO sobre os pontos levantados pelo QA (não-bloqueantes)
1. **Sem `tailwind.config.*`** — aceito. Tailwind v4 usa `@tailwindcss/vite` + diretivas no CSS;
   o critério literal da task ("`tailwind.config.*` presente") nasceu da premissa de Tailwind v3 e
   está obsoleto. Omitir o `preflight` foi a decisão correta — foi o que evitou regressão em `/` e `/about`.
2. **`client/yarn.lock` removido** — aceito. O Dockerfile faz `COPY client/package*.json` + `npm install`;
   o `yarn.lock` estava obsoleto, não era usado no build e atrapalhava o CLI do shadcn.
3. **Critério "`npm test` do client"** — N/A. A premissa da task ("jest/RTL já existente em `client/`")
   estava errada: não há runner de testes no `client/` (só `dev`/`build`/`preview`/`server`).
   Agrupamento validado por script Node + comportamento validado ponta a ponta pelo QA.
   Introduzir Vitest no `client/` fica como item de backlog separado (fora do escopo desta task).
4. **CLI `npx shadcn init/add` quebrado no caminho Vite+JSX** — aceito. O setup foi refeito
   manualmente com o mesmo resultado (componente `chart` oficial do registry, TSX→JSX,
   `THEMES.dark` ajustado para `[data-theme="dark"]`).

### Conflito com a task 006
Quando a task 007 foi implementada, a 006 ainda não tinha sido mergeada. A **006 foi mergeada
(PR #3) em `ia-main` antes da abertura do PR da 007** — as duas alteram `client/src/App.jsx` e
`client/src/index.css`, então o PR da 007 exige **rebase trivial** nesses 2 arquivos.
